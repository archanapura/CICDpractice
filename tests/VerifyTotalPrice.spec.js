...
test('@web Verify total price added', async ({ page }) => {
    ...
    const added_quan=await page.locator('div.product>p.product-price').textContent()
    const total_quant=await page.locator('tr:has(td:has-text("Price"))>td>strong').textContent()
